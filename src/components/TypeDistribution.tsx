...
export default function TypeDistribution() {
  ...
  useEffect(() => {
    ...
    d3.csv("/data/pokmeon_competitive.csv").then((data) => {
      ...
      data.forEach((d) => {
        ...
        const secondary = d.type2?.toLowerCase() === "" ? null : d.type2?.toLowerCase();
        if (typeMap[primary]) {
  ...
  useEffect(() => {
    ...
    types.forEach(type => {
      ...
      const sortedSecondaries = Object.entries(secondaryTypes)
        .sort((a, b) => b[1] - a[1]);
      ...
      sortedSecondaries.forEach(([secondaryType, count]) => {
        ...
        g.append("rect")
          ...
          .attr("opacity", 0.7)
          .append("title")
          .text(`${secondaryType === "No_type" ? "No Secondary Type" : secondaryType}: ${count} Pokémon`);
    ...
    const legendData = Object.entries(typeColors).filter(([type]) => type !== "No_type");
    ...
    g.append("rect")
      ...
      .attr("height", Math.ceil(legendData.length / legendItemsPerRow) * legendItemHeight + legendTitleHeight + 10)
      .attr("fill", "#1a1f2c")
      .attr("opacity", 0.8)
      .attr("rx", 5);
    ...
    for (let i = 0; i < legendData.length; i++) {
      ...
      const legendGroup = g.append("g")
        .attr("transform", `translate(${legendX + col * legendItemWidth}, ${legendY + row * legendItemHeight})`);
