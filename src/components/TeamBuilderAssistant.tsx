...
export default function TeamBuilderBarChart() {
    const [team, setTeam] = useState<(string | null)[]>([ // Team Array
        null,
        null,
        null,
        null,
    ...
    const [typeCounts, setTypeCounts] = useState<Record<string, number>>(initialTypeCounts);

    const [netDefensiveSpread, setNetDefensiveSpread] = useState<Record<string, number>>(
    ...
    const [currentTypeOrder, setCurrentTypeOrder] = useState<string[]>([...filteredTypes]); // Current ordering of types on X-axis
    ...
    useEffect(() => {
      if (isSorted) {
            ...
            const sorted = [...filteredTypes].sort((a, b) => (netDefensiveSpread[b] || 0) - (netDefensiveSpread[a] || 0));
    ...
    useEffect(() => {
        ...
        // Bind data to bars
        const bars = g.selectAll<SVGRectElement, typeof barData[0]>(".bar").data(barData, d => d.type);
        ...
        const barsEnter = bars.enter()
            ...
            .attr("y", yScale(0))
            .attr("height", 0)
            .attr("fill", d => d.value > 0 ? "crimson" : d.value < 0 ? "steelblue" : "gray")
            .attr("opacity", 0.85)
            .on("mouseover", (event, d) => {
                ...
                .html(`
                    ...
                    ${d.value > 0 ? `Weaknesses: +${d.value}` :
                    d.value < 0 ? `Resistances: ${-d.value}` : "Neutral"}
                `);
            })
            .on("mousemove", (event) => {
                tooltip
                    ...
                    {team.map((name, i) => {
                        ...
                        return (
                            ...
                            >
                                {poke ? (
                                    <img
                                        src={`https://play.pokemonshowdown.com/sprites/gen5/${getShowdownSpriteName(poke.name)}.png`}
                                        alt={poke.name}
